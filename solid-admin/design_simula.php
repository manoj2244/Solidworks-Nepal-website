...
switch ($method) {
	...
	case "POST":
		if (isset($data->deletedId)) {
				...
				$deleted = mysqli_query($db_conn, "delete from design_simu where id='$deletedId'");
			...
			$allUsers = mysqli_query($db_conn, "SELECT * FROM design_simu where id = '" . $editid . "'");
			...
			if(isset($_FILES['picture'])){
				...
				$sql = "SELECT * FROM `design_simu` WHERE `id`=$id";
				...
				$edit = mysqli_query($db_conn, "update design_simu set access ='$access', used ='$used',detail='$detail',image='$filename' where id='$id'");
				if ($edit) {
			...
			//if (isset($_POST['title']) && isset($_POST['slogan']) && isset($_POST['youtubeurl'])) {
				...
				$add = mysqli_query($db_conn, "insert into design_simu (access,used,detail,image) values('$access','$used','$detail','$filename')");
				if ($add) {
