...
switch ($method) {
	...
	case "POST":
		if (isset($data->deletedId)) {
				...
				$deleted = mysqli_query($db_conn, "delete from designer where id='$deletedId'");
			...
			$allUsers = mysqli_query($db_conn, "SELECT * FROM designer where id = '" . $editid . "'");
			...
			if(isset($_FILES['picture'])){
				...
				$edit = mysqli_query($db_conn, "update designer set heading ='$heading', name ='$name',designation='$designation',facebook='$facebook',insta='$insta',twitter='$twitter',linkdin='$linkdin',image='$filename' where id='$id'");
				if ($edit) {
			...
			//if (isset($_POST['title']) && isset($_POST['slogan']) && isset($_POST['youtubeurl'])) {
				...
				$add = mysqli_query($db_conn, "insert into designer (heading,name,designation,facebook,insta,twitter,linkdin,image) values('$heading','$name','$designation','$facebook','$insta','$twitter','$linkdin','$filename')");
				if ($add) {
