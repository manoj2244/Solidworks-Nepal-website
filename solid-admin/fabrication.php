...
switch ($method) {
	...
	case "POST":
		if (isset($data->deletedId)) {
				...
				$deleted = mysqli_query($db_conn, "delete from fabrication where id='$deletedId'");
			...
			$allUsers = mysqli_query($db_conn, "SELECT * FROM fabrication where fab_id = '" . $editid . "'");
			...
			$allUsers = mysqli_query($db_conn, "SELECT * FROM fabrication_image where work_id = '" . $editid . "'");
			if (mysqli_num_rows($allUsers) > 0) {
                while ($row = mysqli_fetch_assoc($allUsers)) {
                    $json_data[] = $row;
                }
			...
			$allUsers = mysqli_query($db_conn, "SELECT * FROM fabrication where id = '" . $editid . "'");
			if (mysqli_num_rows($allUsers) > 0) {
                while ($row = mysqli_fetch_assoc($allUsers)) {
                    $json_data[] = $row;
                }
			...
			if(isset($_FILES['picture'])){
				...
				$sql = "SELECT * FROM `fabrication` WHERE `id`=$id";
				...
				$edit = mysqli_query($db_conn, "update fabrication set title ='$title', category ='$category',client='$client',url='$url',project_date='$project_date',description='$desc',image='$filename',main_desc='$main_desc' where id='$id'");
				if ($edit) {
			...
			//if (isset($_POST['title']) && isset($_POST['slogan']) && isset($_POST['youtubeurl'])) {
				...
				//   $fab = substr($captcha,0,10);
                $fab_id = rand(1000000,999999999);

				$add = mysqli_query($db_conn, "insert into fabrication (title,category,project_date,url,client,description,image,main_desc,fab_id) values('$title','$category','$project_date','$url','$client','$desc','$filename','$main_desc','$fab_id')");
				if ($add) {
                    ...
                    for ($i = 0; $i < count($files['name']); $i++) {
                        ...
                        mysqli_query($db_conn, "insert into fabrication_image (image,work_id) values('".$file['name'] ."','$fab_id')");
                      }
