...
switch ($method) {
	...
	case "POST":
		if (isset($data->deletedId)) {
				...
				$deleted = mysqli_query($db_conn, "delete from simulation where id='$deletedId'");
			...
			$allUsers = mysqli_query($db_conn, "SELECT * FROM simulation where simu_id = '" . $editid . "'");
			...
			$allUsers = mysqli_query($db_conn, "SELECT * FROM simulation_image where work_id = '" . $editid . "'");
			if (mysqli_num_rows($allUsers) > 0) {
                while ($row = mysqli_fetch_assoc($allUsers)) {
                    $json_data[] = $row;
                }
			...
			$allUsers = mysqli_query($db_conn, "SELECT * FROM simulation where id = '" . $editid . "'");
			if (mysqli_num_rows($allUsers) > 0) {
                while ($row = mysqli_fetch_assoc($allUsers)) {
                    $json_data[] = $row;
                }
			...
			if(isset($_FILES['picture'])){
				...
				$edit = mysqli_query($db_conn, "update simulation set title ='$title', category ='$category',client='$client',url='$url',project_date='$project_date',description='$desc',image='$filename',main_desc='$main_desc' where id='$id'");
				if ($edit) {
			...
			//if (isset($_POST['title']) && isset($_POST['slogan']) && isset($_POST['youtubeurl'])) {
                ...
                // $simu_ids = substr($captcha,0,10);
                $simu_ids = rand(1000000,999999999);
                


				$add = mysqli_query($db_conn, "insert into simulation (title,category,project_date,url,client,description,image,main_desc,simu_id) values('$title','$category','$project_date','$url','$client','$desc','$filename','$main_desc','$simu_ids')");
				if ($add) {
                    ...
                    for ($i = 0; $i < count($files['name']); $i++) {
                        ...
                        // $query = "INSERT INTO simulation_image (image,work_id) VALUES ('" . $file['name'] . "')";
                        // mysqli_query($db_conn, $query);
    
                        mysqli_query($db_conn, "insert into simulation_image (image,work_id) values('".$file['name'] ."','$simu_ids')");
                      }
